...
import { createAndSendCode, verifyAndSendToken } from "../services/auth/create0tpAndSend";
...
export const Register = async (req: Request, res: Response) => {
    ...
    return res.status(200).json({ success: true, message: 'Account created your OTP is ' + otp });
};
...
export const assignRole = async (req: Request, res: Response) => {
    ...
    await service.findByIdAndUpdate(req.query.id as string, { role: role as string });
...
export const updateProfile = async (req: Request, res: Response) => {
    const user: any = await service.findUserById(req.user.id);
    if(user.role === 'admin'){
        await service.findByIdAndUpdate(req.query.id as string, { ...req.body });
    }
    if(user.role === 'user'){
        await service.findByIdAndUpdate(req.user.id, { ...req.body });
